 ...
 *
 */
public class CancelAsyncSample {
    ...
    public static void main(String... args) {
         ...
         * requests to process.
         ***********************************************************************/
         CancelRequest requestOne = new CancelRequest();
         // @TODO: set request parameters here

         CancelRequest requestTwo = new CancelRequest();
         // @TODO: set second request parameters here

         List<CancelRequest> requests = new ArrayList<CancelRequest>();
         requests.add(requestOne);
         requests.add(requestTwo);

         invokeCancel(service, requests);
     ...
     */
    public static void invokeCancel(AmazonFPSAsync service, List<CancelRequest> requests) {
        List<Future<CancelResponse>> responses = new ArrayList<Future<CancelResponse>>();
        for (CancelRequest fpsRequest : requests) {
            responses.add(service.cancelAsync(fpsRequest));
        }
        for (Future<CancelResponse> future : responses) {
            ...
            try {
                CancelResponse fpsResponse = future.get();
                // Original request corresponding to this response, if needed:
                CancelRequest originalRequest = requests.get(responses.indexOf(future));
