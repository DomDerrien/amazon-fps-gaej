 ...
 *
 */
public class FundPrepaidAsyncSample {
    ...
    public static void main(String... args) {
         ...
         * requests to process.
         ***********************************************************************/
         FundPrepaidRequest requestOne = new FundPrepaidRequest();
         // @TODO: set request parameters here

         FundPrepaidRequest requestTwo = new FundPrepaidRequest();
         // @TODO: set second request parameters here

         List<FundPrepaidRequest> requests = new ArrayList<FundPrepaidRequest>();
         requests.add(requestOne);
         requests.add(requestTwo);

         invokeFundPrepaid(service, requests);
     ...
     */
    public static void invokeFundPrepaid(AmazonFPSAsync service, List<FundPrepaidRequest> requests) {
        List<Future<FundPrepaidResponse>> responses = new ArrayList<Future<FundPrepaidResponse>>();
        for (FundPrepaidRequest fpsRequest : requests) {
            responses.add(service.fundPrepaidAsync(fpsRequest));
        }
        for (Future<FundPrepaidResponse> future : responses) {
            ...
            try {
                FundPrepaidResponse fpsResponse = future.get();
                // Original request corresponding to this response, if needed:
                FundPrepaidRequest originalRequest = requests.get(responses.indexOf(future));
