 ...
 *
 */
public class CancelTokenAsyncSample {
    ...
    public static void main(String... args) {
         ...
         * requests to process.
         ***********************************************************************/
         CancelTokenRequest requestOne = new CancelTokenRequest();
         // @TODO: set request parameters here

         CancelTokenRequest requestTwo = new CancelTokenRequest();
         // @TODO: set second request parameters here

         List<CancelTokenRequest> requests = new ArrayList<CancelTokenRequest>();
         requests.add(requestOne);
         requests.add(requestTwo);

         invokeCancelToken(service, requests);
     ...
     */
    public static void invokeCancelToken(AmazonFPSAsync service, List<CancelTokenRequest> requests) {
        List<Future<CancelTokenResponse>> responses = new ArrayList<Future<CancelTokenResponse>>();
        for (CancelTokenRequest fpsRequest : requests) {
            responses.add(service.cancelTokenAsync(fpsRequest));
        }
        for (Future<CancelTokenResponse> future : responses) {
            ...
            try {
                CancelTokenResponse fpsResponse = future.get();
                // Original request corresponding to this response, if needed:
                CancelTokenRequest originalRequest = requests.get(responses.indexOf(future));
